...
function AuthorProfilePreview() {
  ...
  // Reactive size values - these memos ensure reactivity when sliders change
  ...
  // Element renderer component - uses Show to ensure reactivity for each element
  const ElementRenderer = (props: { id: string }) => {
    return (
      <>
        ...
        <Show when={props.id === 'about' && profileData().about}>
          <p class="text-text-muted line-clamp-2" style={{ 'font-size': `${aboutSize()}px` }}>{profileData().about}</p>
        </Show>

        <Show when={props.id === 'location' && profileData().location}>
          <span class="flex items-center gap-1 text-text-muted" style={{ 'font-size': `${metaSize()}px` }}>
        ...
        <Show when={props.id === 'website' && profileData().website}>
          <a
        ...
        <Show when={props.id === 'followers'}>
          <div class="text-center">
            <p class="font-bold text-text" style={{ 'font-size': `${statsSize()}px` }}>{formatCompactNumber(profileData().followers)}</p>
        ...
        <Show when={props.id === 'following'}>
          <div class="text-center">
            <p class="font-bold text-text" style={{ 'font-size': `${statsSize()}px` }}>{formatCompactNumber(profileData().following)}</p>
        ...
        <Show when={props.id === 'postCount'}>
          <div class="text-center">
            <p class="font-bold text-text" style={{ 'font-size': `${statsSize()}px` }}>{formatCompactNumber(profileData().postCount)}</p>
        ...
        <Show when={props.id === 'votingPower'}>
          <div class="text-center">
            <p class="font-semibold text-text" style={{ 'font-size': `${statsSize()}px` }}>--</p>
        ...
        <Show when={props.id === 'hiveBalance'}>
          <div class="text-center">
            <p class="font-semibold text-text" style={{ 'font-size': `${statsSize()}px` }}>{profileData().hiveBalance.toFixed(3)}</p>
        ...
        <Show when={props.id === 'hbdBalance'}>
          <div class="text-center">
            <p class="font-semibold text-text" style={{ 'font-size': `${statsSize()}px` }}>{profileData().hbdBalance.toFixed(3)}</p>
  ...
  const renderChild = (child: CardSectionChild) => {
    if (child.type === 'element') {
      return <ElementRenderer id={child.id} />
    } else {
